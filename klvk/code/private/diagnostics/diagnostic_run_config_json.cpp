...
{
    const auto field = root.OptionalField("exit");
    if (!field) return;
    const JsonReader& exit = *field;
...
DiagnosticRunConfig DiagnosticRunConfigJson::Read(
    ...
    std::optional<DiagnosticPresentation> presentation_override)
{
    ...
    if (presentation_override) config.presentation = *presentation_override;
