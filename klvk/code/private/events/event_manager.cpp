...
void EventManager::UpdateListenTypes(IEventListener* listener)
{
...

    {
        ...
        for (size_t index = 0; index != types.size(); ++index)
        {
            ...
            {
                klvk::ErrorHandling::Ensure(type, "IEventListener::GetEventTypes returns nullptr!");
                auto callback = listener->MakeCallbackFunction(index);
                klvk::ErrorHandling::Ensure(callback, "IEventListener::MakeCallbackFunction returns nullptr!");
